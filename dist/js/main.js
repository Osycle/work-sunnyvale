...
(function() {
    ...
    //Отдельные буквы
    $("[data-divideword]").map(function(i, el){
    	//if(checkSm()) return;
    	var arrText = $(el).text().match(/(\W)/gim)
    	var stepCnt = $(this).attr("data-divideword");
    	$(this).text("");
    	for (var i = 0; i < arrText.length; i++) {
    		var step = (i%stepCnt)+1;
    		console.log("divideword-"+step);
    		if( arrText[i] == " " )
    				arrText[i] = "&nbsp";
    		$(this).append("<span class='divideword-"+step+"'>"+arrText[i]+"</span>");
    	}
    })
